...
class CifarConvNet(BaseNet):
  ...
  def _build_network(self):
    ...
    network = tl.layers.MaxPool2d(network, (3, 3), (1, 1), padding = 'VALID', name = 'pool2')
    network = tl.layers.Conv2d(network, 64, (3, 3), (1, 1), act = act_fn.swish, padding = 'VALID', name = 'cnn3')
    ...
    network = tl.layers.DenseLayer(network, n_units = 10, act = act_fn.swish, name = 'dense5')
  ...
  def load_dataset(self, filename = None):
    ...
    '''
    x, y = dataloader.load_cifar10_dataset(filename)
    # x = (x / 255.) - 0.5
    return x, y
